...
- 安全覆盖统一放在 `package.json` 的 `pnpm.overrides`（undici、tar、minimatch、serialize-javascript、@babel/*、brace-expansion、js-yaml 等），更新锁文件时注意保留。注意：`pnpm-workspace.yaml` 里的 `overrides` 字段是 pnpm 10 语法，本项目 pnpm 9 不识别，不要写在那里。
