...
├── adConfig.ts           # 广告配置
├── friendsConfig.ts      # 友链配置（页面开关，友链数据通过 Pages CMS 的 CSV 代码编辑器维护）
...
- `friendsConfig.ts` - 友链配置（页面行为开关；友链列表数据存放在 `src/data/friends.csv`，通过 Pages CMS 的 CSV 代码编辑器在线维护，`tags` 列用英文分号分隔多个标签，如 `Blog;Tech`）
